...
- Прозрачность реализуем обычной альфой и градиентами. Рантайм-блур Compose-модификаторов
  не используем (дорого), НО допускаем кэшированное размытие BITMAP-сцены один раз при отрисовке
  экрана: BoxBlur по downscale-копии фото, выполняется в Dispatchers.Default и кладётся в кэш —
  реализация `ui/components/Backdrop.kt` (WorldSceneCache). Поверхность карточек стекла:
  вертикальный градиент `fill(alpha 0.05→0.09)` поверх чёрного + `border(0.5–1dp, белый alpha 0.14)`
  + лёгкий верхний блик (только у верхнего края карточки).
...
- Онбординг: если `!RedShiftState.isOnboarded` — показать онбординг (п. 8.0)
  ...
  И запись флага в SharedPreferences `redshift_ui_prefs / onboarded=true` (персист в
  ui-слое через `LocalContext`, `service/*` не трогаем). При старте флаг читается из prefs.
...
- [ ] Онбординг не блокирует после `isOnboarded=true`; флаг персистится в prefs
      (`redshift_ui_prefs/onboarded`) и не показывается после перезапуска приложения.
